...
def load_live_predictions_from_gcs():
    ...
    blob = bucket.blob("live/live_news_predictions.parquet")
    blob.download_to_filename("/tmp/live_news_predictions.parquet")

    return pd.read_parquet("/tmp/live_news_predictions.parquet")
...
with tab2:
    ...
    try:
        df = load_live_predictions_from_gcs()
    except Exception as e:
        st.info(f"Could not load cloud live predictions: {e}")
